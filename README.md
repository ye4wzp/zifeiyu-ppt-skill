...
- **14 个注册版式 × 2 套设计系统 × 3 套 Token 主题**：版式覆盖封面、目录、章节、金句、对比、论证、KPI、时间线、图文、台账、流程、全幅图、结尾；设计系统有现代无衬线与「墨韵」衬线杂志风（宋体 × 暖纸 × 朱砂）两套——同一版式库，换一个 CSS 文件整套切换
- **四格式交付链**：HTML 演示 → 可编辑 PPTX → 矢量 PDF → 逐页 PNG，全部一条命令，HTML 是唯一的源
- **机器质量门**：Playwright 校验器执行 R1–R9 量化规则（版式注册、类名白名单、溢出、字号下限、文本碰撞、密度带……），错误即阻断交付
- **大字报张力**：100px 封面标题、220px 章节序号、136px 细字重 KPI 数字、280px 半透明鬼影、单页深色节奏
